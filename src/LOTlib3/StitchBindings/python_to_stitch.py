...
def parsed_to_stitch(parsed):
    """
    Convert parsed expressions to Stitch syntax, with special handling for lambda applications.
    """
    if isinstance(parsed, list):
        op, *args = parsed
        
        # Special case: If this is a lambda application
        if op.strip() == 'lam' and len(args) > 1:
            # The lambda body is the first argument
            body = args[0]
            # The application arguments are the rest
            app_args = args[1:]
            
            # Format the lambda with extra parentheses
            lambda_expr = f'((lam {parsed_to_stitch(body)})'
            
            # Add application arguments
            for arg in app_args:
                lambda_expr += f' {parsed_to_stitch(arg)}'
            
            return lambda_expr + ')'
        else:
            # Normal case - regular function call or special form
...
pythonExpr = "lambda x7: (lambda x8: (lambda x9: (lambda x10: dict(__list__(__tuple__(__str__(input))(x9))(__tuple__(__str__(output))(x10))))(x9(x9)))(x7(x7)))"
pythonExpr = "lambda x1: lambda x2: (lambda x3: (lambda x4: (lambda x5: (lambda x6: (lambda x7: (lambda x8: (lambda x9: (lambda x10: __ifExp__(__and__(eq(x9)(x10))(eq(x10)(1)))))))))))"
